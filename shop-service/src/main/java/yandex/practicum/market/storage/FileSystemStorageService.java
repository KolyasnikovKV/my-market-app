package yandex.practicum.market.storage;
...
public class FileSystemStorageService implements StorageService {
    ...
    public void delete(String path) {
        Path destinationDir = this.rootLocation
                ...
                .normalize().toAbsolutePath();
        if (!destinationDir.getParent().startsWith(this.rootLocation.toAbsolutePath())) {
            // This is a security check
            throw new StorageException(
                    "Cannot delete files outside current directory.");
        }

        FileSystemUtils.deleteRecursively(destinationDir.toFile());
    }
